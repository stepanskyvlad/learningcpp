...
{
    std::deque<char> original_order {};
    std::deque<char> reversed_order {};
    for (char c : s) {
        if (std::isalpha(static_cast<unsigned char>(c))) {
            original_order.push_back(std::toupper(c));
            reversed_order.push_front(std::toupper(c));
        }
    }
    // You must implement this function.
    // Since we are learning the STL - use a deque to solve the problem.
    if (original_order == reversed_order) {
        return true;
    } else {
        return false;
    }
